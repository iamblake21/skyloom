...
namespace CML.Editor.Art
{
    ...
    public static class StarterIslandTerrainSetup
    {
        ...
        public const string CliffQaScenePath =
            "Assets/_Project/Scenes/92_StarterIsland_Cliff_QA.unity";
        ...
        private static string renderScenePathOverride;
        ...
        /// <summary>
        /// Rebuilds the production Terrain assets and a deterministic art-only
        /// QA scene.  The QA scene intentionally excludes the optional AIR
        /// gameplay rig so environment iteration cannot be blocked by an
        /// unrelated vehicle prefab, while the normal review-scene pipeline
        /// retains its strict gameplay validation.
        /// </summary>
        private static void RebuildCliffAssetsAndQaScene()
        {
            EnsureFolders();
            var layers = BuildTerrainLayers();
            var terrainData = BuildTerrainData(layers);
            ...
            AssetDatabase.SaveAssets();
            AssetDatabase.Refresh();
            BuildCliffQaScene(prefab);
            AssetDatabase.SaveAssets();
            AssetDatabase.Refresh();
            Debug.Log(
                $"STARTER_ISLAND_CLIFF_QA_BUILD data={TerrainDataPath} " +
                $"prefab={PrefabPath} scene={CliffQaScenePath} " +
                "airshipDependency=0 status=PASS");
        }
        ...
        public static void RenderTerrainCliffCloseBenchmark()
        {
            RenderSceneView(
                ...
                "TERRAIN_CLIFF_CLOSE",
                new Vector3(-126f, 37.50f, 2f),
                new Vector3(-138f, 41.00f, 20f),
                50f,
                null,
                false,
                true);
        ...
        [MenuItem("CML/Art/Render Starter Island Terrain Cliff Ascent Gate")]
        public static void RenderTerrainCliffAscentGate()
        {
            // Fixed gameplay-height view along the authored Portal route. It
            // must expose whether the shelves read as traversable structure,
            // rather than relying on a flattering telephoto hero shot.
            RenderSceneView(
                "CML_STARTER_ISLAND_TERRAIN_CLIFF_ASCENT_RENDER",
                @"D:\CodexTemp\StarterIslandTerrain\terrain_cliff_ascent.png",
                "TERRAIN_CLIFF_ASCENT",
                new Vector3(112f, 45.65f, 34f),
                new Vector3(194f, 67f, 91f),
                67f);
        }

        [MenuItem("CML/Art/Render Starter Island Terrain Cliff Overcast Gate")]
        public static void RenderTerrainCliffOvercastGate()
        {
            RenderSceneView(
                "CML_STARTER_ISLAND_TERRAIN_CLIFF_OVERCAST_RENDER",
                @"D:\CodexTemp\StarterIslandTerrain\terrain_cliff_overcast.png",
                "TERRAIN_CLIFF_OVERCAST",
                new Vector3(-130f, 36.95f, 0f),
                new Vector3(-151f, 42.65f, 34f),
                60f,
                scene =>
                {
                    var sun = FindSceneLight(scene, "ENV_Sun");
                    if (sun != null)
                    {
                        sun.color = Html("#C5D5E2");
                        sun.intensity = 0.52f;
                        sun.shadowStrength = 0.58f;
                    }

                    RenderSettings.ambientSkyColor = Html("#C8D5DA");
                    RenderSettings.ambientEquatorColor = Html("#B9B7B4");
                    RenderSettings.ambientGroundColor = Html("#77747A");
                    RenderSettings.ambientIntensity = 0.78f;
                });
        }

        [MenuItem("CML/Art/Render Starter Island Terrain Cliff Grazing Gate")]
        public static void RenderTerrainCliffGrazingGate()
        {
            RenderSceneView(
                "CML_STARTER_ISLAND_TERRAIN_CLIFF_GRAZING_RENDER",
                @"D:\CodexTemp\StarterIslandTerrain\terrain_cliff_grazing.png",
                "TERRAIN_CLIFF_GRAZING",
                new Vector3(-126f, 37.50f, 2f),
                new Vector3(-138f, 41.00f, 20f),
                50f,
                scene =>
                {
                    ...
                    if (sun != null)
                    {
                        sun.transform.rotation =
                            Quaternion.Euler(13f, -84f, 0f);
                        sun.color = Html("#FFD0A4");
                        sun.intensity = 2.05f;
        ...
        [MenuItem("CML/Art/Render Starter Island Cliff Clay Close Gate")]
        public static void RenderTerrainCliffClayCloseGate()
        {
            RenderSceneView(
                "CML_STARTER_ISLAND_TERRAIN_CLIFF_CLAY_CLOSE_RENDER",
                @"D:\CodexTemp\StarterIslandTerrain\terrain_cliff_clay_close.png",
                "TERRAIN_CLIFF_CLAY_CLOSE",
                new Vector3(-126f, 37.50f, 2f),
                new Vector3(-138f, 41.00f, 20f),
                50f,
                scene => ConfigureCliffClay(scene, grazingLight: false),
                false,
                true);
        }

        [MenuItem("CML/Art/Render Starter Island Cliff Clay Grazing Gate")]
        public static void RenderTerrainCliffClayGrazingGate()
        {
            RenderSceneView(
                "CML_STARTER_ISLAND_TERRAIN_CLIFF_CLAY_GRAZING_RENDER",
                @"D:\CodexTemp\StarterIslandTerrain\terrain_cliff_clay_grazing.png",
                "TERRAIN_CLIFF_CLAY_GRAZING",
                new Vector3(-126f, 37.50f, 2f),
                new Vector3(-138f, 41.00f, 20f),
                50f,
                scene => ConfigureCliffClay(scene, grazingLight: true),
                false,
                true);
        }

        [MenuItem("CML/Art/Render Starter Island Cliff Wireframe Gate")]
        public static void RenderTerrainCliffWireframeGate()
        {
            RenderSceneView(
                "CML_STARTER_ISLAND_TERRAIN_CLIFF_WIREFRAME_RENDER",
                @"D:\CodexTemp\StarterIslandTerrain\terrain_cliff_wireframe.png",
                "TERRAIN_CLIFF_WIREFRAME",
                new Vector3(-126f, 37.50f, 2f),
                new Vector3(-138f, 41.00f, 20f),
                50f,
                scene => ConfigureCliffClay(scene, grazingLight: false),
        ...
        [MenuItem("CML/Art/Render Starter Island Cliff Material Crop Gate")]
        public static void RenderTerrainCliffMaterialCropGate()
        {
            RenderSceneView(
                "CML_STARTER_ISLAND_TERRAIN_CLIFF_CROP_RENDER",
                @"D:\CodexTemp\StarterIslandTerrain\terrain_cliff_crop.png",
                "TERRAIN_CLIFF_CROP",
                new Vector3(-133.8f, 38.80f, 12.5f),
                new Vector3(-137f, 40.20f, 18f),
                32f,
                null,
                false,
                true);
        ...
        {
            var terrainPassed = AuditShader(
                "CML/Environment/Starter Island Terrain Splat",
                "TERRAIN_SPLAT");
            var surfacePassed = AuditShader(
                "CML/Environment/Starter Island Stylized Surface",
                "STYLIZED_SURFACE");

            if (!terrainPassed || !surfacePassed)
            {
                throw new InvalidOperationException(
                    "One or more Starter Island rock shaders failed " +
                    "the support/message audit.");
            }

            Debug.Log(
                "STARTER_ISLAND_ROCK_SHADER_GATE shaders=2 " +
                "status=PASS");
        }

        private static bool AuditShader(string shaderName, string marker)
        {
            var shader = Shader.Find(shaderName);
            if (shader == null)
            {
                Debug.LogError(
                    $"STARTER_ISLAND_ROCK_SHADER_AUDIT shader={marker} " +
            ...
            for (var index = 0; index < messages.Length; index++)
            {
                ...
                Debug.Log(
                    $"STARTER_ISLAND_ROCK_SHADER_MESSAGE shader={marker} " +
                    $"severity={message.severity} " +
                    $"platform={message.platform} " +
                    $"line={message.line} message={message.message}");
            }

            var passed = shader.isSupported && messages.Length == 0;
            Debug.Log(
                $"STARTER_ISLAND_ROCK_SHADER_AUDIT shader={marker} " +
        ...
        [MenuItem("CML/Art/Rebuild And Render Cliff Professional Gate")]
        public static void RunAndRenderCliffProfessionalGate()
        {
            RebuildCliffAssetsAndQaScene();
            RenderCliffProfessionalGate();
        }

        [MenuItem("CML/Art/Render Cliff Professional Gate")]
        public static void RenderCliffProfessionalGate()
        {
            try
            {
                renderScenePathOverride = CliffQaScenePath;
                AuditTerrainShader();
                RenderTerrainCliffQaPair();
                RenderTerrainCliffAscentGate();
                RenderTerrainCliffOvercastGate();
                RenderTerrainCliffGrazingGate();
                RenderTerrainCliffClayCloseGate();
                RenderTerrainCliffClayGrazingGate();
                RenderTerrainCliffWireframeGate();
                RenderTerrainCliffMaterialCropGate();
                RenderUnderbodyView();
                RenderHeroView();
                RenderPortalApproachView();
                RenderPortalCloseView();
            }
            finally
            {
                renderScenePathOverride = null;
            }
        }
        ...
        private static Light FindSceneLight(Scene scene, string objectName)
        {
                 ...
                 rootIndex++)
            {
                ...
                for (var lightIndex = 0;
                     ...
                     lightIndex++)
                {
                    if (lights[lightIndex].gameObject.name == objectName)
                    {
        ...
        private static void ConfigureCliffClay(
            Scene scene,
            bool grazingLight)
        {
            var terrain =
            ...
            {
                var terrainClay = new Material(terrain.materialTemplate)
                {
                    name = "QA_Clay_Terrain"
                };
                SetFloat(terrainClay, "_ClayMode", 1f);
                SetColor(terrainClay, "_ClayColor", Html("#98867C"));
                terrain.materialTemplate = terrainClay;
            }
            ...
            {
                var renderers =
                    roots[rootIndex].GetComponentsInChildren<MeshRenderer>(
                        true);
                     ...
                     rendererIndex++)
                {
                    ...
                    if (!HasAncestorNamed(
                            renderer.transform,
                            StarterIslandCliffFasciaBuilder.RootName) &&
                        !HasAncestorNamed(
                            renderer.transform,
                    ...
                    var clay = new Material(source)
                    {
                        name = "QA_Clay_RockMesh"
                    };
                    SetFloat(clay, "_ClayMode", 1f);
                    SetColor(clay, "_ClayColor", Html("#98867C"));
        ...
        private static void ConfigureCliffWireframe(Scene scene)
        {
            ConfigureCliffClay(scene, grazingLight: false);
            var filters =
            ...
            for (var filterIndex = 0;
                 ...
                 filterIndex++)
            {
                var sourceFilter = filters[filterIndex];
                if (!HasAncestorNamed(
                ...
                var lineIndices = new List<int>(sourceTriangles.Length * 2);
                for (var triangle = 0;
                     ...
                     triangle += 3)
                {
                    AddWireEdge(
                        ...
                        sourceTriangles[triangle + 1],
                        edges,
                        lineIndices);
                    AddWireEdge(
                        sourceTriangles[triangle + 1],
                        sourceTriangles[triangle + 2],
                        edges,
                        lineIndices);
                    AddWireEdge(
                        sourceTriangles[triangle + 2],
                ...
                var wireObject = new GameObject("QA_CliffFasciaWire");
                wireObject.transform.SetParent(
                    sourceFilter.transform,
                    false);
                wireObject.AddComponent<MeshFilter>().sharedMesh = wireMesh;
                var renderer = wireObject.AddComponent<MeshRenderer>();
                var shader = Shader.Find("CML/QA/Starter Island Wire");
                if (shader == null)
                {
                    throw new InvalidOperationException(
                        "Starter Island QA wire shader is unavailable.");
                }
                ...
                SetColor(material, "_BaseColor", Html("#42E6FF"));
            ...
            List<int> lineIndices)
        {
            ...
            lineIndices.Add((int)minimum);
            lineIndices.Add((int)maximum);
        }
        ...
        private static void RenderSceneView(
            ...
            bool requireTerrainWallTarget = false)
        {
            var renderScenePath =
                string.IsNullOrWhiteSpace(renderScenePathOverride)
                    ? ReviewScenePath
                    : renderScenePathOverride;
            if (!File.Exists(
                    Path.Combine(
                        Application.dataPath,
                        renderScenePath.Substring("Assets/".Length))))
            {
                throw new FileNotFoundException(
                    $"Terrain review scene is missing: {renderScenePath}");
            }
            ...
            if (requireTerrainWallTarget)
            {
                Physics.SyncTransforms();
                var requestedTarget = targetPosition;
                var targetDirection =
                    requestedTarget - cameraPosition;
                var requestedDistance = targetDirection.magnitude;
                if (requestedDistance <= 0.001f ||
                    !Physics.Raycast(
                        cameraPosition,
                        targetDirection / requestedDistance,
                        out var wallHit,
                        requestedDistance + 12f,
                        Physics.DefaultRaycastLayers,
                        QueryTriggerInteraction.Ignore) ||
                    !(wallHit.collider is TerrainCollider))
                {
                    throw new InvalidOperationException(
                        $"Terrain {marker} camera ray does not hit the " +
                        "collision-bearing Terrain wall.");
                }

                var targetError = Vector3.Distance(
                    wallHit.point,
                    requestedTarget);
                if (targetError > 5f)
                {
                    throw new InvalidOperationException(
                        $"Terrain {marker} target misses the collider by " +
                        $"{targetError:F2} m.");
                }

                var terrainLocal =
                    wallHit.point - renderTerrain.transform.position;
                var terrainSize = renderTerrain.terrainData.size;
                var terrainSlope =
                    renderTerrain.terrainData.GetSteepness(
                        Mathf.Clamp01(
                            terrainLocal.x /
                            Mathf.Max(terrainSize.x, 0.001f)),
                        Mathf.Clamp01(
                            terrainLocal.z /
                            Mathf.Max(terrainSize.z, 0.001f)));
                if (terrainSlope < 28f)
                {
                    throw new InvalidOperationException(
                        $"Terrain {marker} target slope is only " +
                        $"{terrainSlope:F1} degrees; it is not a cliff.");
                }

                targetPosition = wallHit.point;
                Debug.Log(
                    $"STARTER_ISLAND_TERRAIN_{marker}_TARGET " +
                    $"collider=TerrainCollider slope={terrainSlope:F1} " +
                    $"distance={wallHit.distance:F2} " +
                    $"targetError={targetError:F2} status=PASS");
            }
            ...
            try
            {
                ...
                if (maximumChannel - minimumChannel < 8)
                {
                    throw new InvalidOperationException(
                        $"Terrain {marker} render is blank or invalid " +
                        $"(channelRange=" +
                ...
                for (var x = 0; x < SurfaceTextureResolution; x++)
                {
                    ...
                    if (isCliff)
                    {
                        ...
                        var warpedV =
                            Mathf.Repeat(
                                v + (warp - 0.5f) * 0.090f,
                                1f);
                        var broadStrata = SeamlessNoiseAnisotropic(
                            u,
                            warpedV,
                            2.4f,
                            6.2f,
                            offsetX,
                            offsetY);
                        var brokenPlates = SeamlessNoiseAnisotropic(
                            u,
                            warpedV,
                            6.8f,
                            10.4f,
                            offsetY + 9.1f,
                            offsetX + 4.7f);
                        var fracturedDetail = SeamlessNoise(
                            u,
                            warpedV,
                            22f,
                            offsetX + 17.3f,
                    ...
                    if (isCliff)
                    {
                        ...
                        var compactGrain = SeamlessNoise(
                            ...
                            offsetX + 19.4f);
                        var breakup =
                            (stoneBreakup - 0.5f) * 0.040f +
                            (compactGrain - 0.5f) * 0.015f;
                        color *= 1f + breakup;
        ...
        private static TerrainData BuildTerrainData(TerrainLayer[] layers)
        {
            ...
            RepaintTerrainLayers(data, layers);
            // Ground cover is baked into deterministic mesh chunks by
            // StarterIslandGroundCoverBuilder. Keeping the old Terrain detail
            // prototypes here would render a second copy and, more
            // importantly, would strip the corrective FBX-node transforms
            // that keep the authored grass and flowers upright.
        ...
        private static GameObject BuildTerrainPrefab(TerrainData data)
        {
            ...
            try
            {
                ...
                StarterIslandGroundCoverBuilder.Build(
                    root.transform,
                    terrain);
                ...
                var continuousRockMaterial = BuildUnderbodyMaterial();
                StarterIslandUnderbodyBuilder.BuildOrUpdate(
                    root.transform,
                    underbodyRim,
                    continuousRockMaterial);
                StarterIslandCliffFasciaBuilder.BuildOrUpdate(
                    root.transform,
                    terrain,
                    BuildCliffFasciaMaterial());
        ...
        private static Material BuildTerrainMaterial()
        {
            ...
            SetFloat(material, "_CliffNormalStrength", 0.0f);
            ...
            SetFloat(material, "_CliffReliefNormalStrength", 0.0f);
            SetFloat(material, "_CliffMicroNormalStrength", 0.045f);
            ...
            SetFloat(material, "_CliffStrataStrength", 0.32f);
            SetColor(material, "_CliffLichenColor", Html("#718052"));
            SetFloat(material, "_CliffLichenScale", 0.16f);
            SetFloat(material, "_CliffLichenStrength", 0.22f);
            SetColor(material, "_CliffSoilColor", Html("#5A4638"));
            SetFloat(material, "_CliffSoilStrength", 0.22f);
            SetFloat(material, "_CliffSpecularStrength", 0.028f);
        ...
        private static Material BuildUnderbodyMaterial()
        {
            ...
            SetColor(material, "_BaseColor", Html("#A36E60"));
            SetColor(material, "_SecondaryColor", Html("#755962"));
            SetColor(material, "_WetColor", Html("#453F49"));
            SetFloat(material, "_VertexBlend", 1f);
            SetFloat(material, "_AmbientStrength", 0.50f);
            SetFloat(material, "_ShadowFloor", 0.14f);
            SetFloat(material, "_ColorVariation", 0.075f);
            SetFloat(material, "_RockDetail", 1f);
            SetColor(material, "_RockTopColor", Html("#C99A78"));
            SetColor(material, "_RockUnderColor", Html("#554A55"));
            SetFloat(material, "_RockTopStrength", 0.38f);
            SetFloat(material, "_RockUnderStrength", 0.36f);
            SetFloat(material, "_RockMacroScale", 0.075f);
            SetFloat(material, "_RockMacroStrength", 0.085f);
            SetFloat(material, "_RockGrainScale", 0.82f);
            SetFloat(material, "_RockGrainStrength", 0.020f);
            SetFloat(material, "_RockSpecularStrength", 0.024f);
            SetFloat(material, "_RockSpecularPower", 30f);
            EditorUtility.SetDirty(material);
            return material;
        }

        private static Material BuildCliffFasciaMaterial()
        {
            var shader = Shader.Find(
                "CML/Environment/Starter Island Stylized Surface");
            if (shader == null)
            ...
            var path =
                MaterialsRoot + "/M_StarterIsland_CliffFascia.mat";
            var material = AssetDatabase.LoadAssetAtPath<Material>(path);
            if (material == null)
            {
                material = new Material(shader)
                {
                    name = "M_StarterIsland_CliffFascia"
                };
                AssetDatabase.CreateAsset(material, path);
            }
            else
            ...
            // This is deliberately the same warm sandstone family as the
            // Terrain. Large value changes come from authored planes and the
            // sun; strata, grain and rough specular remain subordinate.
            SetColor(material, "_BaseColor", Html("#A87564"));
            SetColor(material, "_SecondaryColor", Html("#7C5B60"));
            SetColor(material, "_WetColor", Html("#514650"));
            SetFloat(material, "_VertexBlend", 1f);
            SetFloat(material, "_AmbientStrength", 0.58f);
            SetFloat(material, "_ShadowFloor", 0.25f);
            SetFloat(material, "_ColorVariation", 0.045f);
            SetFloat(material, "_RockDetail", 1f);
            SetColor(material, "_RockTopColor", Html("#C99A78"));
            SetColor(material, "_RockUnderColor", Html("#66545A"));
            SetFloat(material, "_RockTopStrength", 0.42f);
            SetFloat(material, "_RockUnderStrength", 0.24f);
            SetFloat(material, "_RockMacroScale", 0.11f);
            SetFloat(material, "_RockMacroStrength", 0.10f);
            SetFloat(material, "_RockGrainScale", 1.65f);
            SetFloat(material, "_RockGrainStrength", 0.035f);
            SetFloat(material, "_RockSpecularStrength", 0.020f);
            SetFloat(material, "_RockSpecularPower", 34f);
            SetFloat(material, "_RockContactBlend", 0f);
            EditorUtility.SetDirty(material);
            return material;
        }
        ...
        {
            BuildEnvironmentReviewScene(
                terrainPrefab,
                ReviewScenePath,
                includePlayableAirship: true);
        }

        private static void BuildCliffQaScene(GameObject terrainPrefab)
        {
            BuildEnvironmentReviewScene(
                terrainPrefab,
                CliffQaScenePath,
                includePlayableAirship: false);
        }

        private static void BuildEnvironmentReviewScene(
            GameObject terrainPrefab,
            string scenePath,
            bool includePlayableAirship)
        {
            ...
            try
            {
                ...
                if (includePlayableAirship)
                {
                    CreatePlayableAirshipRig(
                        island,
                        terrain,
            ...
            float height)
        {
            ...
            height = ApplyWaterChannel(
                worldX, worldZ, height, CrownFallRoute, 2.65f, 7.5f);
            height = ApplyWaterChannel(
                worldX, worldZ, height, ThirdCreekRoute, 2.55f, 10.0f);
            height = ApplyWaterChannel(
                worldX, worldZ, height, ThirdFallRoute, 2.60f, 7.5f);
            height = ApplyWaterChannel(
                worldX, worldZ, height, MiddleCreekRoute, 2.60f, 10.0f);
            height = ApplyWaterChannel(
                worldX, worldZ, height, MiddleFallRoute, 2.70f, 8.0f);
            height = ApplyWaterChannel(
                worldX, worldZ, height, ShelfCreekRoute, 2.70f, 11.0f);
            height = ApplyWaterChannel(
                worldX, worldZ, height, MainFallRoute, 3.00f, 9.0f);
